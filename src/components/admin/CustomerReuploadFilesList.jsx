...
import { createSignedCustomerFileUrl } from '../../services/additionalInfoAdminService';
...
export default function CustomerReuploadFilesList({ files }) {
  ...
  const handleDownload = async (file) => {
    ...
    setError('');
    try {
      const url = await createSignedCustomerFileUrl(file.file_path);
  ...
  if (!files.length) {
    return (
      <div className="rounded-lg border border-dashed border-slate-200 bg-slate-50 px-4 py-6 text-center text-sm text-metallic">
        No customer re-upload files yet.
      </div>
    );
  }
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                ...
                {file.uploaded_by_email && (
                  <p className="mt-1 text-xs text-metallic">Uploaded by {file.uploaded_by_email}</p>
                )}
              </div>
              <button
                type="button"
